...
def _get_batch(generator, batch_size, num_steps, max_word_length):
    ...
    while True:
        ...
        for i in range(batch_size): ## i is a sentence it seems, batch size is the number of sentences in each text
            cur_pos = 0
...
class LMDataset(object):
    ...
    def __init__(self, filepattern, vocab, reverse=False, test=False,
        ...
        """
        self._vocab = vocab
        self._all_shards = [filepattern + f.strip() for f in os.listdir(filepattern)] ##  if f.endswith('gz')
        # for i in self._all_shards:
        #     print(i)
        # self._all_shards = glob.glob(filepattern + '*.gz')
        print('Found %d shards at %s (always 1 for good-bad classification)' % (len(self._all_shards), filepattern))
    ...
    def _load_shard(self, shard_name):
        ...
        ### smart-open should cope with .txt as well as .gz if I remove the filter for fns above: a URI for the local filesystem: `./lines.txt`
