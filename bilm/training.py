...
class LanguageModel(object):
    ...
    def _build_word_char_embeddings(self):
        ...
        if n_chars != 261: ## the number that was there at download 261
...
def test(options, ckpt_file, data, batch_size=1):
    ...
    ### session seems to be cleared at each iteration
    with tf.Session(config=config) as sess:
        ## this is where parallelism can be implemented: see https://keras.io/getting-started/faq/#how-can-i-run-a-keras-model-on-multiple-gpus
        with tf.device('/gpu:0'), tf.variable_scope('lm',reuse=tf.AUTO_REUSE): ## I added the parameter here
        ...
        # model.total_loss is the op to compute the loss, i.e. softmax_cross_entropy
        ...
        for batch_no, batch in enumerate(
            ...
            feed_dict.update(_get_feed_dict_from_x(x, 0, x['token_ids'].shape[0], model,
                                      char_inputs, bidirectional)
            )
